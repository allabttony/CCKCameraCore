...
@implementation UIImage (CCKImageProcess)
...
+ (void)processOriginalImage:(UIImage *)originalImage widthResolution:(CGFloat)widthResolution whRadio:(CGFloat)radio success:(CCKSnapSuccessBlock)success
{
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        CGFloat width = widthResolution;
        CGFloat height = width / radio;
        
        CGSize sizeOriginal = CGSizeMake(width, height);
        UIImage *scaledImageOriginal = [originalImage resizedImageWithContentMode:UIViewContentModeScaleAspectFill bounds:sizeOriginal interpolationQuality:kCGInterpolationHigh];
        ...
        NSData *jpegData = UIImageJPEGRepresentation(croppedImageOriginal, 0.7f);
        NSLog(@"jpeg: %ld(kb)", (long)jpegData.length / 1024);
        UIImage *jpegImage = [UIImage imageWithData:jpegData];
        success(jpegImage, jpegData);
    });
}
